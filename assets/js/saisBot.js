...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  let knowledgeDefault =
    "Maaf, saya belum paham. Coba tanya tentang:\n- Profil Joko\n- Proyek portofolio\n- Cara menghubungi Joko\n- Tentang AI ini";
  ...
  function addMessage(sender, message, messageId = null) {
    ...
    avatarElement.src =
      sender === "bot"
        ? "assets/images/logo-sais-blue.png"
        : "assets/images/person.png";
    avatarElement.alt = sender === "bot" ? "SAIS Bot Logo" : "User Avatar";
    avatarElement.classList.add(
      "border",
      sender === "bot" ? "border-pink-500" : "border-blue-500"
    );
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        if (knowledgeSentences.length > 0) {
          ...
          normalizedKnowledgeEmbeddings = tf.div(
            embeddings,
        ...
        addMessage(
          "bot",
          data.initial_greeting || "Halo! Ada yang bisa saya bantu?"
        );
      } else {
  ...
  async function getBotResponse(userMessage) {
    ...
    try {
      ...
      const normalizedUserEmbedding = tf.div(
        userEmbedding,
        tf.norm(userEmbedding)
      );
      ...
      const threshold = Math.max(
        0.7,
        0.85 - knowledgeSentences.length * 0.00005
      );
